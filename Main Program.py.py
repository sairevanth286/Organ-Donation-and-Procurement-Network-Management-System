...
def add_User():
    ...
    for field in fields:
        ...
        if field not in ['User_ID', 'Medical_insurance'] and temp != '':
            temp = temp
        if temp == '':
            temp = None  # Use None for NULL in SQL
        
        val.append(temp)

    # Assuming the number of placeholders matches the number of columns
    qry = "INSERT INTO User (%s) VALUES (%s)" % (', '.join(fields), ', '.join(['%s'] * len(fields)))
    print(qry)
